...
def _fmt_ty(value: float | int) -> str:
    """Format a raw VND amount as 'X.XXX ty' (billions)."""
    ty = value / 1e9
    if ty >= 100:
        int_ty = round(ty)
        formatted = f"{int_ty:,}".replace(",", ".")
        return f"{formatted} ty"
    elif ty >= 10:
        return f"{ty:,.1f} ty".replace(",", ".")
    else:
        return f"{ty:,.2f} ty".replace(",", ".")
...
class _TreasuryPDF(FPDF):
    ...
    def __init__(self) -> None:
        ...
        self.add_font("DejaVu", "", os.path.join(_FONTS_DIR, "DejaVuSans.ttf"), uni=True)
        self.add_font("DejaVu", "B", os.path.join(_FONTS_DIR, "DejaVuSans-Bold.ttf"), uni=True)
        self.add_font("DejaVu", "I", os.path.join(_FONTS_DIR, "DejaVuSans-Oblique.ttf"), uni=True)
        self.add_page()
...
def generate_summary_pdf() -> bytes:
    ...
    pdf.cell(0, 7, f"Ngay bao cao: {report_date}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
